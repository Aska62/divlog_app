...
import updateDivePlan from '@/actions/divePlan/updateDivePlan';
...
const EditDivePlanPage:React.FC<EditPlanPageProps> = ({ params }) => {
  ...
  const [state, formAction, isPending] = useActionState(updateDivePlan, {});
  ...
  return (
    <div className={`${!isScrollable && 'h-screenWOHeader overflow-y-hidden'}`}>
      <Heading pageTitle={"Edit Dive Plan"} />
      { isLoading || isPending ? <>Loading...</> :
        <>
          <form action={formAction} className="w-11/12 max-w-xl h-fit mx-auto my-12">
            <input type="hidden" name="id" value={ divePlan.id || '' } readOnly />
...
export default EditDivePlanPage;